...
export class CatVisualizerEngine {
    constructor(canvasElement) {
        ...
        this.models = { cyber: new CyberCat(), cosmic: new CosmicCat() };
        ...
        this.activeCat = (savedModel === "cyber" || savedModel === "cosmic") ? savedModel : "cyber";
        ...
        this.catCenterX = this.width * 0.50;
        ...
        PlaybackStateSynchronizer.register(this, "cat");

        this.loop = this.loop.bind(this);
        if (this.isForeground) {
            PlaybackStateSynchronizer.broadcastActivation("cat");
            this.updateAllUI();
        }
    }
    ...
    toggleActive(forcedState = null) {
        ...
        if (this.isForeground) PlaybackStateSynchronizer.broadcastActivation("cat");
        if (typeof localStorage !== "undefined") localStorage.setItem("cosmic-cat-bg-enabled", this.isForeground ? "true" : "false");
        if (this.canvas) this.canvas.style.display = this.isForeground ? "block" : "none";
    ...
    setCat(modelIdentifier) {
        if (modelIdentifier !== "cyber" && modelIdentifier !== "cosmic") return this.activeCat;
        this.activeCat = modelIdentifier;
        if (typeof localStorage !== "undefined") localStorage.setItem("cosmic-cat-model", modelIdentifier);
        this.updateAllUI();
    ...
    nextCat() {
        return this.setCat(this.activeCat === "cyber" ? "cosmic" : "cyber");
    }

    setPalette(paletteIdentifier) {
        this.paletteManager.setPalette(paletteIdentifier, true);
        if (typeof localStorage !== "undefined") localStorage.setItem("cosmic-cat-theme", paletteIdentifier);
        this.updateAllUI();
        if (this.isForeground && this.isFrozen) this.renderFrame(0.016, true);
    }
    ...
    nextPalette() {
        ...
        if (typeof localStorage !== "undefined" && nextPalette && nextPalette.id) {
            localStorage.setItem("cosmic-cat-theme", nextPalette.id);
        }
        this.updateAllUI();
        if (this.isForeground && this.isFrozen) this.renderFrame(0.016, true);
        return nextPalette;
    ...
    loop(currentTimestampMs) {
        ...
        try {
            ...
            console.error("[CatVisualizerEngine] Animation loop error:", loopError);
        } finally {
...
export const VisualizerEngine = CatVisualizerEngine;
